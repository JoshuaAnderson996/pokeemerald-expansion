...
static bool32 CanBreakThroughAbility(u32 battlerAtk, u32 battlerDef, u32 ability, u32 hasAbilityShield, u32 ignoreMoldBreaker);
...
static enum MoveCanceler CancelerParalyzed(void)
{
    if (!gBattleStruct->isAtkCancelerForCalledMove
        ...
        && !IsAbilityAndRecord(gBattlerAttacker, GetBattlerAbility(gBattlerAttacker), ABILITY_CLERIC)
        && !IsAbilityAndRecord(gBattlerAttacker, GetBattlerAbility(gBattlerAttacker), ABILITY_CEREBRAL_SANCTUARY)
...
bool32 CanAbilityBlockMove(u32 battlerAtk, u32 battlerDef, u32 abilityAtk, u32 abilityDef, u32 move, enum FunctionCallOption option)
{
    ...
    case ABILITY_SOUNDPROOF:
        if (IsSoundMove(move) && !(GetBattlerMoveTargetType(battlerAtk, move) & MOVE_TARGET_USER)
            && IsBatSpecies(gBattleMons[battlerDef].species))
        {
    ...
    case ABILITY_CALAMITY_EDGE:
        ...
        {
            if (gBattleMons[battlerAtk].status1 & VOLATILE_MULTIPLETURNS)
                gHitMarker |= HITMARKER_NO_PPDEDUCT;
            battleScriptBlocksMove = BattleScript_SoundproofProtected;
        }
        break;
...
bool32 CanAbilityAbsorbMove(u32 battlerAtk, u32 battlerDef, u32 abilityDef, u32 move, u32 moveType, enum FunctionCallOption option)
{
    ...
    {
    default:
        // No script to run for this outcome
        break;
    ...
    case MOVE_ABSORBED_BY_DRAIN_HP_ABILITY:
        ...
        {
            battleScript = (gProtectStructs[battlerAtk].notFirstStrike)
    ...
    case MOVE_ABSORBED_BY_BOOST_EXORCIST:
        ...
        if (!gDisableStructs[battlerDef].exorcistBoosted)
        {
            ...
            battleScript = (gProtectStructs[battlerAtk].notFirstStrike)
                         ? BattleScript_ExorcistBoost
                         : BattleScript_ExorcistBoost_PPLoss;
...
u32 AbilityBattleEffects(u32 caseID, u32 battler, u32 ability, u32 special, u32 moveArg)
{
    ...
    case ABILITYEFFECT_SWITCH_IN_WEATHER:
        ...
        if (effect != 0)
    {
        // Show “The rain continues to fall!” etc. without starting durations
        // or attributing the weather to this Pokémon’s ability.
        BattleScriptPushCursor();
        gBattlescriptCurrInstr = BattleScript_OverworldWeatherStarts;
        return effect; // (matches the usual AbilityBattleEffects pattern)
    }
    break;
...
break;
        ...
        case ABILITY_GULP_MISSILE:
             ...
             && gBattleMons[gBattlerTarget].species != SPECIES_CRAMORANT)
            {
                ...
                && GetBattlerAbility(gBattlerAttacker) != ABILITY_CEREBRAL_SANCTUARY
                && GetBattlerAbility(gBattlerAttacker) != ABILITY_CLERIC)
                {
                    gBattleStruct->moveDamage[gBattlerAttacker] = GetNonDynamaxMaxHP(gBattlerAttacker) / 4;
                    if (gBattleStruct->moveDamage[gBattlerAttacker] == 0)
                        gBattleStruct->moveDamage[gBattlerAttacker] = 1;
...
u32 IsAbilityPreventingEscape(u32 battler)
{
    ...
    for (u32 battlerDef = 0; battlerDef < gBattlersCount; battlerDef++)
    {
        ...
        if (ability == ABILITY_PHANTOM_ONSLAUGHT)
            return battlerDef + 1;

        if (ability == ABILITY_PHANTOM_AEGIS)
            return battlerDef + 1;
    }
...
u32 ItemBattleEffects(enum ItemCaseId caseID, u32 battler)
{
    ...
    case ITEMEFFECT_TRY_HEALING:
        if (gBattleMons[battler].hp)
        {
            ...
            case HOLD_EFFECT_BLACK_SLUDGE:
                ...
                && !IsAbilityAndRecord(battler, GetBattlerAbility(battler), ABILITY_CLERIC)
                && !IsAbilityAndRecord(battler, GetBattlerAbility(battler), ABILITY_CEREBRAL_SANCTUARY))
                {
                    gBattleStruct->moveDamage[battler] = GetNonDynamaxMaxHP(battler) / 8;
                    if (gBattleStruct->moveDamage[battler] == 0)
                        gBattleStruct->moveDamage[battler] = 1;
        ...
        if (!(gBattleStruct->moveResultFlags[gBattlerTarget] & MOVE_RESULT_NO_EFFECT))
        {
            ...
            case HOLD_EFFECT_ROCKY_HELMET:
                if (IsBattlerTurnDamaged(gBattlerTarget)
                    ...
                    && GetBattlerAbility(gBattlerAttacker) != ABILITY_CEREBRAL_SANCTUARY
                    && GetBattlerAbility(gBattlerAttacker) != ABILITY_RECKLESS
    ...
    case ITEMEFFECT_ORBS:
    {
        ...
        case HOLD_EFFECT_STICKY_BARB:   // Not an orb per se, but similar effect, and needs to NOT activate with pickpocket
            if (battlerAbility != ABILITY_MAGIC_GUARD && battlerAbility != ABILITY_CLERIC && battlerAbility != ABILITY_CEREBRAL_SANCTUARY)
            {
                gBattleStruct->moveDamage[battler] = GetNonDynamaxMaxHP(battler) / 8;
                if (gBattleStruct->moveDamage[battler] == 0)
                    gBattleStruct->moveDamage[battler] = 1;
...
static inline u32 CalcMoveBasePower(struct DamageContext *ctx)
{
    ...
    {
    case EFFECT_HIT:
        // No special base power calculation needed for EFFECT_HIT
        break;
    case EFFECT_PLEDGE:
    ...
    case EFFECT_ABSORB:
        /* Draining moves (Absorb, Giga Drain, etc.) don't modify base power here */
        break;
    case EFFECT_NON_VOLATILE_STATUS:
        /* Moves that only apply status effects don't modify base power */
        break;
    case EFFECT_DREAM_EATER:
        /* Dream Eater doesn't modify base power here */
        break;
    case EFFECT_MIRROR_MOVE:
        /* Mirror Move uses the base power of the original move */
        break;  
    ...
    case EFFECT_PLACEHOLDER:
        ...
        default:
        // Any other EFFECT_* does not change base power
        break;
    }
...
static inline u32 CalcMoveBasePowerAfterModifiers(struct DamageContext *ctx)
{
    ...
    case ABILITY_SPACE_AGE:
        if (moveType == TYPE_COSMIC && gBattleStruct->battlerState[battlerAtk].ateBoost)
            modifier = uq4_12_multiply(modifier, UQ_4_12(1.2));
        break;    
    ...
    case ABILITY_SIEGE_SPECIALIST:
        if (gSideStatuses[GetBattlerSide(battlerDef)] & (HAZARDS_SPIKES
    ...
    case ABILITY_EXPLORER:
        ...
        modifier = uq4_12_multiply(modifier, UQ_4_12(1.5)); 
        break;  
    case ABILITY_WEATHER_WARRIOR:
    case ABILITY_FORECAST:
    if (ctx->weather & (B_WEATHER_SUN | B_WEATHER_RAIN | B_WEATHER_SANDSTORM | B_WEATHER_HAIL))
        modifier = uq4_12_multiply(modifier, UQ_4_12(1.3));
    break;
    case ABILITY_MEDIC:
    case ABILITY_CLERIC:
...
else
{
    ...
    case ABILITY_ILLUSION:
{
    // Illusion is considered "active" if state == ILLUSION_ON and we still have a disguise target.
    if (gBattleStruct->illusion[battlerAtk].state == ILLUSION_ON
        && gBattleStruct->illusion[battlerAtk].mon != NULL)
    {
        // 1.5× Attack while disguised
        modifier = uq4_12_multiply(modifier, UQ_4_12(1.5));
    }
    break;
    }
    ...
    case ABILITY_ORICHALCUM_PULSE:
        ...
        break;
    case ABILITY_DRACONIC_FLARE:
        if ((ctx->weather & B_WEATHER_SUN) && HasWeatherEffect() && IsBattleMoveSpecial(move))
           modifier = uq4_12_multiply(modifier, UQ_4_12(1.3333));
        break;
    case ABILITY_HADRON_ENGINE:
    ...
    case ABILITY_AQUA_GODDESS:
    case ABILITY_PRIMORDIAL_SEA:
        if ((ctx->weather & B_WEATHER_RAIN) && HasWeatherEffect() && IsBattleMoveSpecial(move))
            modifier = uq4_12_multiply(modifier, UQ_4_12(1.3333));
        break;
    case ABILITY_DESERT_TYRANT:
        ...
        break;
    case ABILITY_FROST_MONARCH:
        if ((ctx->weather & (B_WEATHER_HAIL | B_WEATHER_SNOW)) && HasWeatherEffect() && IsBattleMoveSpecial(move))
            modifier = uq4_12_multiply(modifier, UQ_4_12(1.3333));
        break;
    case ABILITY_CUTE_CHARM:
    // Boost if the defender is specifically infatuated with THIS attacker
    if (gBattleMons[battlerDef].volatiles.infatuation & INFATUATED_WITH(battlerAtk))
    {
        // 1.3× to whichever offensive stat is being computed
        modifier = uq4_12_multiply(modifier, UQ_4_12(1.3));
    }
    break;
    ...
    /* Pikachu and its costume/forms get the original 2x boost */
    if (atkBaseSpeciesId == SPECIES_PIKACHU
        || atkBaseSpeciesId == SPECIES_PIKACHU_ROCK_STAR
        || atkBaseSpeciesId == SPECIES_PIKACHU_POP_STAR
        || atkBaseSpeciesId == SPECIES_PIKACHU_BELLE
        || atkBaseSpeciesId == SPECIES_PIKACHU_PHD
        || atkBaseSpeciesId == SPECIES_PIKACHU_LIBRE)
    {
        modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(2.0));
    }
    else if ((atkBaseSpeciesId == SPECIES_RAICHU
             || atkBaseSpeciesId == SPECIES_RAICHU_ALOLA
             ...
             || atkBaseSpeciesId == SPECIES_TOGEDEMARU
             || atkBaseSpeciesId == SPECIES_MORPEKO
             || atkBaseSpeciesId == SPECIES_PAWMOT)
            && (B_LIGHT_BALL_ATTACK_BOOST >= GEN_4 || IsBattleMoveSpecial(move)))
    {
        modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
    }
    break;
...
static inline uq4_12_t GetSameTypeAttackBonusModifier(struct DamageContext *ctx)
{
    if (ctx->moveType == TYPE_MYSTERY)
        ...
        if (ctx->abilityAtk == ABILITY_ADAPTABILITY)
            return UQ_4_12(2.0);
        if (ctx->abilityAtk == ABILITY_FOCUSED_ASSAULT)
            return UQ_4_12(1.75);
        return UQ_4_12(1.5);
...
static inline uq4_12_t GetPoisonStatDropModifier(struct DamageContext *ctx, u32 abilityDef)
{
    u32 battlerDef = ctx->battlerDef;
    u32 move       = ctx->move;
    ...
    if ((gBattleMons[battlerDef].status1 & STATUS1_POISON)
        ...
        && abilityDef != ABILITY_CEREBRAL_SANCTUARY
        && abilityDef != ABILITY_CLERIC
...
static inline uq4_12_t GetAttackerAbilitiesModifier(u32 battlerAtk, uq4_12_t typeEffectivenessModifier, bool32 isCrit, u32 abilityAtk)
{
    ...
    case ABILITY_FOCUSED_ASSAULT:
    case ABILITY_IRIDESCENT_EYES:
    case ABILITY_STRIKER:
...
static inline uq4_12_t GetDefenderAbilitiesModifier(struct DamageContext *ctx)
{
    ...
    case ABILITY_SEA_GUARDIAN:
        ...
        break;
        case ABILITY_CEREBRAL_SANCTUARY:
        if (IsBattlerAtMaxHp(ctx->battlerDef)
        ...
        break;
        case ABILITY_PHANTOM_AEGIS:
    // If the attacker is trapped (wrapped / prevented from escaping / rooted),
    // reduce the damage they deal by 30%.
    if (gBattleMons[ctx->battlerAtk].volatiles.escapePrevention)
    {
        modifier = uq4_12_multiply(modifier, UQ_4_12(0.7)); // 30% damage reduction
    }
    break;
    }
...
uq4_12_t CalcPartyMonTypeEffectivenessMultiplier(u16 move, u16 speciesDef, u16 abilityDef)
{
    ...
    if (move != MOVE_STRUGGLE && moveType != TYPE_MYSTERY)
    {
        ...
        /* Abilities that make the move do no damage under certain move types / field statuses */
        if (
            /* Ground moves ignored by Levitate or floating species when Gravity is not active */
            ((ctx.moveType == TYPE_GROUND
             && (abilityDef == ABILITY_LEVITATE || IsFloatingSpecies(speciesDef))
             && !(gFieldStatuses & STATUS_FIELD_GRAVITY)))

            /* Water / Ice moves ignored by Evaporate */
            || ((ctx.moveType == TYPE_WATER || ctx.moveType == TYPE_ICE)
                && abilityDef == ABILITY_EVAPORATE)

            /* Fire moves ignored by Heatproof */
            || (ctx.moveType == TYPE_FIRE && abilityDef == ABILITY_HEATPROOF)
        ) 
        {
            modifier = UQ_4_12(0.0);
        }

        /* Wonder Guard prevents damage from non-super-effective moves (and only if the move has power) */
...
static const u16 sBatSpeciesList[] =
{
    SPECIES_WOOBAT,
    SPECIES_SWOOBAT,
    SPECIES_ZUBAT,
    ...
    bool32 IsBatSpecies(u16 species)
{
    ...
    for (i = 0; i < ARRAY_COUNT(sBatSpeciesList); i++)
    {
        if (species == sBatSpeciesList[i])
            return TRUE;
    }
    return FALSE;
}
...
bool32 CanMoveSkipAccuracyCalc(u32 battlerAtk, u32 battlerDef, u32 abilityAtk, u32 abilityDef, u32 move, enum FunctionCallOption option)
{
    ...
    else if ((abilityAtk == ABILITY_NO_GUARD || abilityAtk == ABILITY_UNERRING_FORCE)
    ...
    else if ((abilityDef == ABILITY_NO_GUARD || abilityDef == ABILITY_UNERRING_FORCE)
          && (moveEffect != EFFECT_SKY_DROP || gBattleStruct->skyDropTargets[battlerDef] == SKY_DROP_NO_TARGET))
    {
        effect = TRUE;
        ability = abilityDef;
...
{
    if ((GetGenConfig(GEN_CONFIG_POWDER_OVERCOAT) >= GEN_6 && (ability == ABILITY_OVERCOAT || ability == ABILITY_DAMP))
