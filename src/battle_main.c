...
const struct TrainerClass gTrainerClasses[TRAINER_CLASS_COUNT] =
{
    ...
    [TRAINER_CLASS_MEGA_MASTER] = { _("MEGA MASTER") },
};
...
u32 GetBattlerTotalSpeedStatArgs(u32 battler, u32 ability, enum ItemHoldEffect holdEffect)
{
    ...
    if (HasWeatherEffect())
    {
        if (ability == ABILITY_SWIFT_SWIM  && gBattleWeather & B_WEATHER_RAIN)
            ...
            speed *= 2;
        else if (ability == ABILITY_PIERCING_DRILL   && gBattleWeather & B_WEATHER_SANDSTORM)
            speed *= 2;
...
u32 GetDynamicMoveType(struct Pokemon *mon, u32 move, u32 battler, enum MonState state)
{
    ...
    case EFFECT_WEATHER_BALL:
        if (state == MON_IN_BATTLE)
        {
            ...
            else if (GetBattlerAbility(battler) == ABILITY_MEGA_SOL)
            return TYPE_FIRE;
        }
        else
        {
    ...
    if (IsSoundMove(move) && ability == ABILITY_CHITTERING_CHORUS)
    {
        return TYPE_BUG;
    }
